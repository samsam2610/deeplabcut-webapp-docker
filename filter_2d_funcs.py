...
from anipose.common import make_process_fun, natural_keys, load_pose_2d, write_pose_2d
...
def process_session_filter_2d(config, session_path):
    ...
    output_root = os.path.join(session_path, pipeline_pose_filter)

    # Recursive search for source files
    if model_type == 'deeplabcut':
        pose_files = glob(os.path.join(pose_folder, '**', '*.h5'), recursive=True)
    elif model_type == 'sleap':
        pose_files = glob(os.path.join(pose_folder, '**', '*.predictions.slp'), recursive=True)
        
    pose_files = sorted(pose_files, key=natural_keys)

    for fname in pose_files:
        # 1. Get the path relative to the input 'pose-2d' folder
        # e.g., if fname is 'pose-2d/cam1/video.h5', rel_path is 'cam1/video.h5'
        rel_path = os.path.relpath(fname, pose_folder)
        
        # 2. Construct the full output path
        outpath = os.path.join(output_root, rel_path)
        
        # 3. Create the necessary subdirectories in the filtered folder
