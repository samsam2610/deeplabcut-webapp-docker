...
# Source: refinedlc/position_filter.py
#
# MIT License
...
try:
    ...
    medcouple = None  # adj_iqr will raise at call time if invoked
...
def process_file(infile: str, output_dir: str, method: str, mode: str,
                 threshold: float = None,
                 mad_threshold: float = None,
                 iqr_multiplier: float = None,
                 std_threshold: float = None,
                 percentile: float = None,
                 log_transform: bool = False):
    """Convenience wrapper: derive outfile from output_dir + input filename."""
    outfile = os.path.join(output_dir, Path(infile).name)
    position_filter(
        infile=infile,
        outfile=outfile,
        method=method,
