    ...
    function _gmRenderJobs(data) {
      ...
      gmJobsList.innerHTML = data.jobs.map(j => {
        ...
        <div style="display:flex;align-items:center;gap:.55rem;padding:.45rem .65rem;background:var(--surface-2);border:1px solid var(--border);border-radius:6px">
          ...
          <span style="font-size:.78rem;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${j.config_path||""}">${j.project||j.engine||"?"}</span>
          <span style="font-size:.72rem;color:var(--text-dim);flex-shrink:0">${j.engine||""}</span>
          <span style="font-size:.72rem;color:var(--text-dim);flex-shrink:0">${ago}</span>
          ${canStop ? `<button class="btn-sm btn-danger gm-stop-btn" data-task-id="${j.task_id}" style="padding:.15rem .45rem;font-size:.72rem;flex-shrink:0" title="Stop this training job">
