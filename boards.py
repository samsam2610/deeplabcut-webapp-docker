...
def merge_rows(all_rows, cam_names=None):
    ...
    assert cam_names is None or len(all_rows) == len(
        cam_names), "number of rows does not match the number of camera names"
...
def reverse_merge_rows(merged):
    """Reverses the merge_rows function and reconstructs the all_rows and cam_names variables.
    Takes the merged variable as input.
    Returns the all_rows and cam_names variables.
    """
    ...
    for row in merged:
        for cam_name, cam_data in row.items():
            if cam_name not in cam_names:
                cam_names.append(cam_name)
                all_rows.append([cam_data])
            else:
                cam_idx = cam_names.index(cam_name)
                all_rows[cam_idx].append(cam_data)

    return all_rows, cam_names


def extract_points(merged, board, cam_names=None, min_cameras=1, min_points=4, check_rtvecs=True):
    ...
    imgp = np.full((n_cams, n_detects, n_points_per_detect, 2), np.nan, dtype='float64')
    
    rvecs = np.full((n_cams, n_detects, n_points_per_detect, 3), np.nan, dtype='float64')
    
    tvecs = np.full((n_cams, n_detects, n_points_per_detect, 3), np.nan, dtype='float64')
    
    objp = np.empty((n_detects, n_points_per_detect, 3), dtype='float64')
    
    board_ids = np.empty((n_detects, n_points_per_detect), dtype='int32')
    ...
    for rix, row in enumerate(merged):
        ...
        for cix, cname in enumerate(cam_names):
            if cname in row:
                ...
                if row[cname].get('rvec', None) is None or row[cname].get('tvec', None) is None:
                    if check_rtvecs:
                        continue
                    else:
    ...
    extra = {'objp': objp, 'ids': board_ids, 'rvecs': rvecs, 'tvecs': tvecs}
    
    return imgp, extra


def reverse_extract_points(imgp, extra, cam_names=None):
    """Reverses the extract_points function and reconstructs the merged variable.
    Takes image points (imgp) and the extra dictionary as inputs.
    ...
    for rix in range(n_detects):
        row = {}
        
        for cix, cname in enumerate(cam_names):
            filled = imgp[cix, rix]
            num_points = np.sum(~np.isnan(filled[:, 0]))
            
            if num_points == 0:
                continue
            
            row[cname] = {'filled': filled[:num_points], 'rvec': extra['rvecs'][cix, rix, :num_points],
                'tvec': extra['tvecs'][cix, rix, :num_points]}
...
def extract_rtvecs(merged, cam_names=None, min_cameras=1, board=None, cameras=None):
    """Takes a list of merged rows (output of merge_rows) and a board object.
        ...
        for cix, cname in enumerate(cam_names):
            if cname in row:
                ...
                if 'rvec' not in r or 'tvec' not in r:
                    if board is None:
                        raise ValueError('rvec or tvec not found in rows. '
                                         'board.estimate_pose_rows should have been run on '
                                         'the rows before merging.'
                                         'If not, the board and cameras must be passed as arguments.')
                    else:
                        rvec, tvec = board.estimate_pose_points(cameras[cix], r['corners'], r['ids'])
                        r['rvec'] = rvec
                        r['tvec'] = tvec
...
class CalibrationObject(ABC):
    ...
    def detect_images(self, images, progress=False, prefix=None):
        ...
        for framenum in it:
            ...
            if corners is not None:
                ...
                row = {'framenum': key, 'corners': corners, 'ids': ids, 'fname': imname}
    ...
    def estimate_pose_rows(self, camera, rows):
        for row in rows:
            rvec, tvec = self.estimate_pose_points(camera, row['corners'], row['ids'])
            row['rvec'] = rvec
            row['tvec'] = tvec
        return rows
...
class Checkerboard(CalibrationObject):
    DETECT_PARAMS = cv2.CALIB_CB_NORMALIZE_IMAGE + cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FAST_CHECK
    ...
    def detect_image(self, image, subpix=True):
        ...
        if corners is not None and self.manually_verify and not self.manually_verify_board_detection(gray, corners):
            corners = None
    ...
    def estimate_pose_points(self, camera, points, ids=None):
        ...
        try:
            retval, rvec, tvec, inliers = cv2.solvePnPRansac(obj_points, points, K, D, confidence=0.9,
...
class CharucoBoard(CalibrationObject):
    def __init__(self, squaresX, squaresY, square_length, marker_length, marker_bits=4, dict_size=50, aruco_dict=None,
        ...
        ARUCO_DICTS = {(4, 50): aruco.DICT_4X4_50, (5, 50): aruco.DICT_5X5_50, (6, 50): aruco.DICT_6X6_50,
            (7, 50): aruco.DICT_7X7_50, (4, 100): aruco.DICT_4X4_100, (5, 100): aruco.DICT_5X5_100,
            (6, 100): aruco.DICT_6X6_100, (7, 100): aruco.DICT_7X7_100, (4, 250): aruco.DICT_4X4_250,
            (5, 250): aruco.DICT_5X5_250, (6, 250): aruco.DICT_6X6_250, (7, 250): aruco.DICT_7X7_250,
            (4, 1000): aruco.DICT_4X4_1000, (5, 1000): aruco.DICT_5X5_1000, (6, 1000): aruco.DICT_6X6_1000,
    ...
    def detect_markers(self, image, camera=None, refine=True):
        ...
        try:
            corners, ids, rejectedImgPoints = aruco.detectMarkers(gray, self.dictionary, parameters=params)
        except Exception:
            ids = None
        ...
        if refine:
            detectedCorners, detectedIds, rejectedCorners, recoveredIdxs = aruco.refineDetectedMarkers(gray, self.board,
                                                                                                       corners, ids,
                                                                                                       rejectedImgPoints,
                                                                                                       K, D,
                                                                                                       parameters=params)
    ...
    def estimate_pose_points(self, camera, corners, ids):
        ...
        ret, rvec, tvec = aruco.estimatePoseCharucoBoard(corners, ids, self.board, K, D, None, None)
