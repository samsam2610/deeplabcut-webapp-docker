...
class TestFindConsecutiveSequences:
    ...
    def test_two_separated_runs(self):
        from dlc_tapnet_tracker import find_consecutive_sequences
        names = self._names([10, 11, 12, 20, 21, 22])
        seqs = find_consecutive_sequences(names)
        assert len(seqs) == 2
        # Verify by abs frame numbers; seq indices continue across the input list
        get_abs = lambda fn: int(re.search(r'-(\d+)\.png', fn).group(1))
        assert [get_abs(f) for f in seqs[0]] == [10, 11, 12]
        assert [get_abs(f) for f in seqs[1]] == [20, 21, 22]
